...
neff_guess = 2.3 - 0i; % Guess effective index of the sought mode
nsol = 20; % number of eigenmodes to find 
...
for i = 1 : ny    
    for j = 1 : nx        
        if (y_i(i) > - wg_height/2) && (y_i(i) < wg_height/2) ...
                && (x_i(j) >= -wg_width/2) && (x_i(j) <= wg_width/2)                
                epsr(i,j) = eps_core;                
        else                 
                epsr(i,j) = eps_clad;
        end
...
box_printf(['THE WAVEGUIDE HEIGHT SPANS ',...
               num2str(nnz(epsr(:,floor(nx/2)) == eps_core)),...
               ' GRID POINTS\n\n',...
              'THE WAVEGUIDE RADIUS SPANS ',...
